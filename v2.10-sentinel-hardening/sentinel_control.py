...
class NonceStore(Protocol):
    """Consume only a previously issued, unexpired, exact-bound grant.

    A set of previously seen caller-supplied tokens is NOT an approval store.
    Issuance must be restricted to an authenticated approval service.
    """
...
async def authorize_execution(
    *,
    ...
    """
    if (isinstance(review_timeout_seconds, bool)
        or not isinstance(review_timeout_seconds, (int, float))
        or not math.isfinite(review_timeout_seconds)
        or not 0 < review_timeout_seconds <= 60):
        raise Denied("invalid_review_timeout")
    ...
    # Reject invalid confirmations before spending evaluator work.
    _verify_confirmation(
        confirmation, principal_id=principal.principal_id,
        proposal_digest=proposal_digest, now=_utcnow(),
    )
    ...
    try:
        raw_review = await asyncio.wait_for(
            evaluator.review(context), timeout=review_timeout_seconds,
        )
        review = EvaluationResult.from_mapping(raw_review)
    except asyncio.TimeoutError as exc:
        raise Denied("evaluator_timeout") from exc
    ...
    # Storage and review waits may outlive the approval. Never reuse entry time.
    _verify_confirmation(
        confirmation, principal_id=principal.principal_id,
        proposal_digest=proposal_digest, now=_utcnow(),
    )
